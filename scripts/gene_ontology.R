...
main = function(universe_path, diffexp_path, go_anno_path, diffexp_direction,
    ...
    if (nrow(diffexp_results) > 0){
        diffexp_results %<>%
            group_by(name) %>%
            arrange(desc(log10_padj), .by_group=TRUE) %>%
            dplyr::slice(1) %>%
            ungroup()

        all_genes = universe %>%
            left_join(diffexp_results, by="name") %>%
        ...
        results = goseq(pwf, gene2cat=go_anno) %>%
            as_tibble() %>%
        ...
        combined_plot = function(df, enrichment_direction){
            ggplot(data = df, aes(y=-log10(pval), x=term, fill=ontology)) +
                ...
                ggtitle(paste(enrichment_direction, "gene ontology terms"),
                        subtitle = paste("transcripts", diffexp_direction, "in", condition, "vs.", control)) +
                theme_light() +
